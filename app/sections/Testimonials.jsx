  ...
  return (
    <div className="bg-white py-12 lg:py-20">
      <div className="relative text-center">
        <h1 className=" text-[17px] lg:text-[35px] font-medium text-rental_primary relative z-10">What People Say About Us</h1>
        <span className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-[40px] lg:text-[6rem] font-bold text-rental_black/25 opacity-30">
          TESTIMONIALS
        </span>
      </div>

      <div className="flex flex-col items-center text-center mt-9 lg:mt-20 px-3 lg:px-6 bg-rental_beige_3 w-[90%] lg:w-[60%] mx-auto py-6 rounded-xl shadow-xl overflow-hidden">
        <div
          className={`transition-transform duration-300 ease-in-out ${
            slide ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0'
          }`}
        >
          <div className="w-16 lg:w-24 h-16 lg:h-24 relative overflow-hidden mx-auto rounded-full">
            <Image src={testimonials[currentIndex].img} alt={testimonials[currentIndex].name} layout="fill" objectFit="cover" />
          </div>
          <p className="mt-6 text-[13px] lg:text-lg text-rental_black max-w-2xl font-light">{testimonials[currentIndex].desc}</p>
          <h2 className="mt-4 text-[14px] lg:text-xl font-semibold text-rental_primary">{testimonials[currentIndex].name}</h2>
        </div>
      </div>
      ...
      <div className="flex justify-center mt-6 gap-2">
        {testimonials.map((_, index) => (
          <span
            key={index}
            className={`h-1 lg:h-2 rounded-full cursor-pointer transition-all ${
