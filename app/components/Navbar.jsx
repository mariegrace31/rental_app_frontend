  ...
  return (
    <nav className={`fixed w-full z-20 flex justify-between p-8 items-center transition-all duration-300 ${scrolled ? 'bg-rental_beige_3 shadow-md' : 'bg-transparent'}`}>
      ...
      <div className='flex gap-4'>
        {['/', '/About', '/Reservations', '/Properties'].map((href, index) => (
          <Link key={index} href={href} className={`text-[19px] font-extralight py-1 px-4 hover:bg-rental_primary hover:text-rental_beige_3 hover:rounded-3xl transition-all duration-300 ${scrolled ? 'text-rental_black' : 'text-rental_beige_3'}`}>
            {href === '/' ? 'Home' : href.slice(1).replace('-', ' ')}
          </Link>
        ))}
        <a href="#contact" className={`text-[18px] font-extralight py-1 px-4 hover:bg-rental_primary hover:text-rental_beige_3 hover:rounded-3xl transition-all duration-300 ${scrolled ? 'text-rental_black' : 'text-rental_beige_3'}`}>
          Contact Us
        </a>
      </div>
